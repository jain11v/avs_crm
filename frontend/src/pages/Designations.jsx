...
export default function Designations() {
  ...
  const load = useCallback(async (q, deptId, p) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Could not load designations.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleDeactivate(designation) {
    ...
    if (!window.confirm(`${verb} ${designation.title}?`)) return;

    try {
      await api.patch(`/designations/${designation.id}/status`, { is_active: nextActive });
      load(search, departmentId, page);
    } catch (err) {
      alert(err.response?.data?.error || `Could not ${verb.toLowerCase()} designation.`);
    }
  }

  async function handleDelete(designation) {
    if (!window.confirm(`Delete ${designation.title}? This cannot be undone.`)) return;

    try {
      await api.delete(`/designations/${designation.id}`);
      load(search, departmentId, page);
    } catch (err) {
      alert(err.response?.data?.error || 'Could not delete designation.');
    }
  }

  const totalPages = Math.max(Math.ceil(total / PAGE_SIZE), 1);
  const addHref = departmentId ? `/designations/new?department_id=${departmentId}` : '/designations/new';
  ...
  return (
    <Layout>
      ...
      {loading ? (
        ...
        <p className="subtitle">No designations found.</p>
      ) : (
        <>
          <table className="data-table">
            ...
            <tbody>
              {rows.map((d) => (
                <tr key={d.id}>
                  <td>
                    <button className="row-link" onClick={() => navigate(`/designations/${d.id}/edit`)}>
                      {d.title}
                    </button>
                  </td>
